...
void sha3_update( sha3_context *priv, void const *bufIn, size_t len)
{
    ...
    DDEBUG_PRN("called to update with:", buf, len);

    assert(ctx->byteIndex < 8);
    assert(ctx->wordIndex < sizeof(ctx->s) / sizeof(ctx->s[0]));

    if(len < old_tail) {        /* have no complete word or haven't started 
                                 * the word yet */
        DEBUG_PRN("because %d<%d, store it and return", (unsigned)len, (unsigned)old_tail);
    ...
    if(old_tail) {              /* will have one word to process */
        DDEBUG_PRN("completing one word with %d bytes", (unsigned)old_tail);
        /* endian-independent code follows: */
        len -= old_tail;
        while (old_tail--)
            ctx->saved |= (uint64_t) (*(buf++)) << ((ctx->byteIndex++) * 8);
    ...
    DDEBUG_PRN("have %d full words to process", (unsigned)words);
    ...
    DDEBUG_PRN("have %d bytes left to process, save them", (unsigned)tail);

    /* finally, save the partial word */
    assert(ctx->byteIndex == 0 && tail < 8);
    while (tail--) {
        DDEBUG_PRN("Store byte %02x '%c'", *buf, *buf);
        ctx->saved |= (uint64_t) (*(buf++)) << ((ctx->byteIndex++) * 8);
    }
    assert(ctx->byteIndex < 8);
    DDEBUG_PRN("Have saved=0x016 PRIx64  at the end %x", ctx->saved);
}
...
void const * sha3_finalize( sha3_context *priv)
{
    ...
    DDEBUG_PRN("called with %d bytes in the buffer", ctx->byteIndex);
    ...
    DEBUG_PRN("Hash (first 128bits): " \
            "%02x%02x%02x%02x %02x%02x%02x%02x - %02x%02x%02x%02x %02x%02x%02x%02x",
            ctx->sb[ 0], ctx->sb[ 1], ctx->sb[ 2], ctx->sb[ 3], 
            ctx->sb[ 4], ctx->sb[ 5], ctx->sb[ 6], ctx->sb[ 7], 
            ctx->sb[ 8], ctx->sb[ 9], ctx->sb[10], ctx->sb[11], 
            ctx->sb[12], ctx->sb[13], ctx->sb[14], ctx->sb[15] 
    );
